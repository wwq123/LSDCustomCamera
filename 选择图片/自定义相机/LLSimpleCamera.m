...
@property (strong, nonatomic) AVCaptureStillImageOutput *stillImageOutput;//用于输出图像
@property (strong, nonatomic) AVCaptureSession *session;//沟通输入与输出之间的桥梁,输入输出设备之间的数据都通过此类传递
@property (strong, nonatomic) AVCaptureDevice *videoCaptureDevice;//视频设备
@property (strong, nonatomic) AVCaptureDevice *audioCaptureDevice;//音频设备
@property (strong, nonatomic) AVCaptureDeviceInput *videoDeviceInput;//视频图像输入
@property (strong, nonatomic) AVCaptureDeviceInput *audioDeviceInput;//音频输入
@property (strong, nonatomic) AVCaptureVideoPreviewLayer *captureVideoPreviewLayer;//镜头捕捉到得预览图层
@property (strong, nonatomic) UITapGestureRecognizer *tapGesture;
@property (strong, nonatomic) CALayer *focusBoxLayer; //焦点图层
@property (strong, nonatomic) CAAnimation *focusBoxAnimation;
@property (strong, nonatomic) AVCaptureMovieFileOutput *movieFileOutput;//用于输出视频
@property (strong, nonatomic) UIPinchGestureRecognizer *pinchGesture;
@property (nonatomic, assign) CGFloat beginGestureScale;
@property (nonatomic, assign) CGFloat effectiveScale;//缩放倍数
...
- (void)viewDidLoad
{
    ...
    //pinch to zoom
    if (_zoomingEnabled) {//添加缩放手势来调整对焦倍数
...
{
    BOOL allTouchesAreOnThePreviewLayer = YES;//检查所有触摸点是否在preview中
    NSUInteger numTouches = [recognizer numberOfTouches];
    for (int i = 0; i < numTouches; ++i ) {
        CGPoint location = [recognizer locationOfTouch:i inView:self.preview];
        CGPoint convertedLocation = [self.preview.layer convertPoint:location fromLayer:self.view.layer];//把self.view中的坐标转化成preview中的坐标
    ...
    if (allTouchesAreOnThePreviewLayer) {
        ...
        if (_effectiveScale < 1.0f)
            _effectiveScale = 1.0f;
        if (_effectiveScale > self.videoCaptureDevice.activeFormat.videoMaxZoomFactor)//判断缩放倍数是否超过设备的最大有效缩放倍数
            _effectiveScale = self.videoCaptureDevice.activeFormat.videoMaxZoomFactor;
        NSError *error = nil;
        if ([self.videoCaptureDevice lockForConfiguration:&error]) {//根据缩放比例调整设备的对焦倍数
...
- (void)start
{
    [LLSimpleCamera requestCameraPermission:^(BOOL granted) {//先检查是否有相机的访问权限
        if(granted) {
...
{
    /*
     1.设置AVCaptureSession桥梁
     2.设置预览图层
     3.设置摄像头方向
     4.根据摄像头方向初始化硬件设备
     5.初始化图像输入并添加到session桥梁中
     6.初始化图像输出并添加到session桥梁中
     7.如果视频可用。则初始化视频输入添加到session桥梁中
     8.如果视频可用。则初始化视频输出添加到session桥梁中
     9.检测预览图层实际协调输入输出的connection是否可用，置为可用
     */
    if(!_session) {
        _session = [[AVCaptureSession alloc] init];
        _session.sessionPreset = self.cameraQuality;//会话质量
        // preview layer
        CGRect bounds = self.preview.layer.bounds;
        _captureVideoPreviewLayer = [[AVCaptureVideoPreviewLayer alloc] initWithSession:self.session];
        _captureVideoPreviewLayer.videoGravity = AVLayerVideoGravityResizeAspectFill;//设置预览图层的显示方式:这里是按宽高比等比例的填充
        _captureVideoPreviewLayer.bounds = bounds;
        _captureVideoPreviewLayer.position = CGPointMake(CGRectGetMidX(bounds), CGRectGetMidY(bounds));//设置描点
        [self.preview.layer addSublayer:_captureVideoPreviewLayer];
        
        AVCaptureDevicePosition devicePosition; //前置后置摄像头切换
...
-(void)capture:(void (^)(LLSimpleCamera *camera, UIImage *image, NSDictionary *metadata, NSError *error))onCapture exactSeenImage:(BOOL)exactSeenImage animationBlock:(void (^)(AVCaptureVideoPreviewLayer *))animationBlock
{
    ...
    [self.stillImageOutput captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler: ^(CMSampleBufferRef imageSampleBuffer, NSError *error) {
        ...
        if (imageSampleBuffer != NULL) {
            ...
            if(self.fixOrientationAfterCapture) {//调整图片的方向
...
- (void)previewTapped:(UIGestureRecognizer *)gestureRecognizer
{
    ...
    CGPoint pointOfInterest = [self convertToPointOfInterestFromViewCoordinates:touchedPoint //将屏幕触摸点坐标转化成预览图层坐标系
...
- (void)focusAtPoint:(CGPoint)point
{
    ...
    if (device.isFocusPointOfInterestSupported && [device isFocusModeSupported:AVCaptureFocusModeAutoFocus]) {//设备是否支持聚焦并且支持给定的对焦模式
        NSError *error;
        if ([device lockForConfiguration:&error]) {//请求设备访问权限以配置设备硬件属性(锁定)，如果为Yes，配置设备硬件属性
            device.focusPointOfInterest = point;
            device.focusMode = AVCaptureFocusModeAutoFocus;
            [device unlockForConfiguration];//修改完后再解锁，否则崩溃
        } else {
            [self passError:error];
        }
    }
...
{
    if(self.focusBoxLayer) {//如果聚焦图层存在，先移动图层到点击位置
        // clear animations
    ...
    if(self.focusBoxAnimation) {//如果图层显示动画存在，在继续执行显示动画
        // run the animation
...
- (AVCaptureVideoOrientation)orientationForConnection
{
    ...
    if(self.useDeviceOrientation) {//是否禁止设备转向
...
/*
 当设备即将转向时会先调用该方法，再调用viewWillLayoutSubviews重新布局子控件frame
 */
