...
typedef enum : NSUInteger {//相机镜头方位:前置or后置
    LLCameraPositionRear,
    LLCameraPositionFront
} LLCameraPosition;

typedef enum : NSUInteger {//闪光灯模式:开or关or自动，默认是关
    LLCameraFlashOff,
    LLCameraFlashOn,
    LLCameraFlashAuto
} LLCameraFlash;
...
typedef enum : NSUInteger {//错误状态信息
    LLSimpleCameraErrorCodeCameraPermission = 10,//没有相机权限
    LLSimpleCameraErrorCodeMicrophonePermission = 11,//没有麦克风权限
